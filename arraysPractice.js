...
function divider(arr) {
  var newArr = [];
  newArr.push(arr.filter(function(a) {
    return a % 2 === 0;
  }));
  newArr.push(arr.filter(function(a) {
    return a % 2 !== 0;
...
function addTen(arr) {
  var newArr = arr.map(Number).map(function(x) {
    return x + 10;
  });
  return newArr;
}
...
return arr1.length === Math.max(arr1.length, arr2.length) ? arr1 : arr2;
}
...
var user2 = {
  ...
  password: 'happyforever',
  username: 'happyalways'
}
